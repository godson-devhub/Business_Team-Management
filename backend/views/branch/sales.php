<?php

use yii\helpers\Html;
use yii\helpers\Url;

/**
 * @var \common\models\Branch $branch
 * @var \common\models\SaleItem[] $sales
 */

$this->title = $branch->name . ' Sales History';

$totalItems = count($sales);
$totalQty = 0;
$totalSalesValue = 0;
$totalProfit = 0;

foreach ($sales as $item) {
    $lineTotal = $item->subtotal ?? ($item->quantity * $item->selling_price);
    $totalQty += $item->quantity;
    $totalSalesValue += $lineTotal;
    $totalProfit += ($item->profit ?? 0);
}
?>

<div class="page-container">

    <!-- Breadcrumb -->
    <nav class="breadcrumb">
        <a href="<?= Url::to(['branch/view', 'id' => $branch->id]) ?>">
            <i data-lucide="chevron-left" class="icon-16"></i>
            <?= Html::encode($branch->name) ?>
        </a>
        <span class="breadcrumb-separator">/</span>
        <span class="breadcrumb-current">Sales History</span>
    </nav>

    <!-- Page Header -->
    <div class="page-header">
        <div>
            <h1 class="page-title">Sales History</h1>
            <p class="page-subtitle">Complete sales transaction record</p>
        </div>
    </div>

    <!-- Summary Cards -->
    <div class="stats-row">
        <div class="stat-card">
            <div class="stat-icon" style="background: rgba(59,130,246,0.15); color: #3b82f6;">
                <i data-lucide="receipt" class="icon-20"></i>
            </div>
            <div class="stat-info">
                <div class="stat-value"><?= number_format($totalItems) ?></div>
                <div class="stat-label">Total Items</div>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon" style="background: rgba(34,197,94,0.15); color: #22c55e;">
                <i data-lucide="layers" class="icon-20"></i>
            </div>
            <div class="stat-info">
                <div class="stat-value"><?= number_format($totalQty) ?></div>
                <div class="stat-label">Quantity Sold</div>
            </div>
        </div>
        <div class="stat-card highlight-blue">
            <div class="stat-icon" style="background: rgba(59,130,246,0.15); color: #3b82f6;">
                <i data-lucide="banknote" class="icon-20"></i>
            </div>
            <div class="stat-info">
                <div class="stat-value" style="color: #3b82f6;">TZS <?= number_format($totalSalesValue) ?></div>
                <div class="stat-label">Sales Value</div>
            </div>
        </div>
        <div class="stat-card highlight-green">
            <div class="stat-icon" style="background: rgba(34,197,94,0.15); color: #22c55e;">
                <i data-lucide="trending-up" class="icon-20"></i>
            </div>
            <div class="stat-info">
                <div class="stat-value" style="color: #22c55e;">TZS <?= number_format($totalProfit) ?></div>
                <div class="stat-label">Total Profit</div>
            </div>
        </div>
    </div>

    <!-- Data Table -->
    <div class="data-card">
        <div class="data-header">
            <h3 class="data-title">
                <i data-lucide="receipt" class="icon-18"></i>
                Sales Transactions
            </h3>
            <span class="data-count"><?= $totalItems ?> records</span>
        </div>

        <?php if (!empty($sales)): ?>

            <div class="table-responsive">
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>Product</th>
                            <th class="text-center">Qty</th>
                            <th class="text-right">Price</th>
                            <th class="text-right">Subtotal</th>
                            <th class="text-right">Profit</th>
                            <th class="text-center">Sale ID</th>
                            <th>Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($sales as $item): ?>
                            <?php $subtotal = $item->subtotal ?? ($item->quantity * $item->selling_price); ?>
                            <tr>
                                <td>
                                    <div class="product-cell">
                                        <div class="product-icon">
                                            <i data-lucide="box" class="icon-16"></i>
                                        </div>
                                        <span class="product-name"><?= Html::encode($item->product->name ?? 'N/A') ?></span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="badge badge-info"><?= $item->quantity ?></span>
                                </td>
                                <td class="text-right mono">TZS <?= number_format($item->selling_price) ?></td>
                                <td class="text-right mono">TZS <?= number_format($subtotal) ?></td>
                                <td class="text-right">
                                    <span class="badge badge-success">TZS <?= number_format($item->profit ?? 0) ?></span>
                                </td>
                                <td class="text-center">
                                    <span class="mono text-muted">#<?= $item->sale_id ?></span>
                                </td>
                                <td class="text-muted">
                                    <?= date('d M Y H:i', $item->sale->created_at ?? time()) ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            </div>

        <?php else: ?>

            <div class="empty-state">
                <div class="empty-icon">
                    <i data-lucide="receipt" class="icon-48"></i>
                </div>
                <h3>No sales found</h3>
                <p>No sales records exist for this branch</p>
            </div>

        <?php endif; ?>

    </div>

</div>

<style>
/* Same styles as daily-sales.php */
</style>

<script>
document.addEventListener('DOMContentLoaded', function() {
    if (typeof lucide !== 'undefined') lucide.createIcons();
});
</script>